 ...
 * MC_Post_Form
 *
 * form for min-calendar post type.
 */
class MC_Post_Form
{
	/**
	...
	private function form( $post_wrapper )
	{
		...
		if ( false === $post_wrapper->initial ) {
			$html .= '<p class="tagcode">' . PHP_EOL
				. esc_html(
					__( "Copy this code and paste it into your post, page or text widget content.", 'mincalendar' )
				)
				. '<br>' . PHP_EOL
		...
		if ( current_user_can( 'edit', $post_id ) ) {
			$html .= '<div class="save-mincalendar">' . PHP_EOL
				. '<input type="submit" class="button-primary" name="mincalendar-save" value="' . esc_attr(
					__( 'Save', 'mincalendar' )
				) . '" />' . PHP_EOL
				. '</div>';
		}
		...
		if ( current_user_can( 'edit', $post_id ) && false === $post_wrapper->initial ) {
			...
			$html .= '<div class="actions-link">' . PHP_EOL
				. '<input type="submit" name="mincalendar-copy" class="copy" value="' . esc_attr(
					__( 'Copy', 'mincalendar' )
				) . '"'
				. " onclick=\"this.form._wpnonce.value = '" . $copy_nonce . "'; this.form.action.value = 'copy'; return true;\"" . " />"
				. '&nbsp;|&nbsp;'
				// delete
				. '<input type="submit" name="delete" class="delete" value="' . esc_attr(
					__( 'Delete', 'mincalendar' )
				) . '"'
