 ...
 * MC_Calendar_Maker
 *
 * Construct drawing markup from min-calendar post id
 */
class MC_Calendar_Maker
{
	...
	public static function draw( $post_id )
	{
		...
		$res         = MC_Day::get_days( $year, $month, false );
		$days        = $res['days'];
...
HTML;
		...
		for ( $i = 1 - $w; $i <= $n + 7; $i ++ ) {
			...
			if ( ( 0 < $i ) && ( $i <= $n ) ) {
				...
				if ( isset( $options['mc-value-1st'] ) && $option === 'mc-value-1st' ) {
					...
					if ( isset( $options['mc-value-2nd'] ) && $option === 'mc-value-2nd' ) {
						$html .= self::set_holiday( '2nd', $youbi );
						$context = ( "\x20" === $options['mc-value-2nd'] ) ? '&nbsp;' : $options['mc-value-2nd'];
					} else {
						if ( isset( $options['mc-value-3rd'] ) && $option === 'mc-value-3rd' ) {
				...
				if ( is_numeric( $relate_posts[$i] ) ) {
					...
					$link   = '<a target="_blank" href="' . get_permalink(
							$relate_posts[$i]
						) . '">' . $relate->post_title . '</a>';
				} else {
					$link = '';
	...
	/**
	 * set style to Sunday and Saturday
	 *
	 * @param $index (1st, 2nd, 3rd)
	 * @param $youbi 0:sunday, 6:saturday
